...
{
    public class ShipInfoManager:BaseManager,IShipInfoManager
    {
        ...
        {
            return _shipInfoRepository.All().Include(x => x.ControlShipInfo).Include(x => x.CommonCode).Include(x => x.CommonCode1).Include(x => x.CommonCode2).ToList();
        }
        ...
        public ResponseModel Saving(ShipInfo shipInfo)
        {
            ...
            if (oldData != null)
            {
                ...
                //oldData.NextDockingDate = shipInfo.NextDockingDate;
                //oldData.NextRefitDate = shipInfo.NextRefitDate;
        ...
        {
            return _shipInfoRepository.Filter(x => x.ControlShipInfoId == id).Include(x => x.ControlShipInfo).Include(x => x.CommonCode).Include(x => x.CommonCode1).Include(x => x.CommonCode2).ToList();
        }
